...
# MAGIC # MLflow
# MAGIC
# MAGIC [MLflow](https://mlflow.org/docs/latest/concepts.html) address these 3 issues:
# MAGIC
# MAGIC * Keep track of experiments
# MAGIC * Reproduce code
# MAGIC * Standardize the packaging and deploying of models
# MAGIC
...
with mlflow.start_run(run_name="LR-Log-Price") as run:
    ...
    # Log artifact creating an histogram
    plt.clf()

    log_train_df.toPandas().hist(column="log_price", bins=100)
    fig = plt.gcf()
    mlflow.log_figure(fig, f"sergio_log_normal.png")
    plt.show()
